from langchain_community.vectorstores import FAISS
from langchain_huggingface import HuggingFaceEmbeddings
from langchain.tools import tool
from backend.config import SYMPTOM_FAISS_DB
from backend.utils.embeddings import get_embeddings


class DiseaseMatcher:
    def __init__(self):

        self.embeddings = get_embeddings()

        self.vectorstore = FAISS.load_local(
            SYMPTOM_FAISS_DB, self.embeddings, allow_dangerous_deserialization=True
        )

    def match(self, query):
        query = ",".join(query)
        results = self.vectorstore.similarity_search(query, k=3)
        final = "symptom matched disease: "
        for r in results:
            print(f"Disease: {r.metadata['disease']}, Text: {r.page_content}")
            final += f"{r.metadata['disease']},"
        return final


matcher = DiseaseMatcher()


@tool
def match_disease_info(query: str) -> str:
    """
    Matches symptoms to diseases and returns possible matches.
    """
    symptoms = [s.strip() for s in query.split(",")]
    return matcher.match(symptoms)
